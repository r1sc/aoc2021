pub fn main(data: Vec<&str>) -> (u32, u32) {
    ...
    let bins: Vec<_> = data.into_iter().map(|l| u32::from_str_radix(&l[..], 2).unwrap()).collect();
    ...
    fn trav<F>(search_vec: &Vec<u32>, f: F, current_bit: usize, bit_width: usize) -> u32
    where
        F: Fn(usize, usize) -> bool,
    {
        ...
        let (ones, zeroes): (Vec<u32>, Vec<u32>) =
            search_vec.into_iter().partition(|f| ((**f >> (bit_width - 1 - current_bit)) & 1) == 1);
    ...
    (epsilon * gamma, oxygen_generator_rating * co2_scrubber_rating)
}
